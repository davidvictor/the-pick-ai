...
/**
 * NBA game generator
 * This class generates mock NBA games
 */
export class NBAGameGenerator extends GameGenerator {
  /**
  ...
  /**
   * Generate a specified number of NBA games
   * @param count Number of games to generate
   * @returns Array of NBA game data
   */
  generateGames(count: number): GameCardProps[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Generate game ID
      const gameId = this.generateGameId("NBA", i);
      ...
      // Generate over/under
      const overUnder = this.generateRandomOverUnder("NBA");
      ...
      const date = this.generateRandomDate(
        mockDataConfig.dateRanges.NBA.start,
        mockDataConfig.dateRanges.NBA.end
      );
      const time = this.generateRandomTime();
      const timezone = this.generateRandomTimezone();
      
      // Generate random network
      const network = this.generateRandomNetwork("NBA");
