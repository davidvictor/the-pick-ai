...
/**
 * MLB game generator
 * This class generates mock MLB games
 */
export class MLBGameGenerator extends GameGenerator {
  /**
  ...
  /**
   * Generate a specified number of MLB games
   * @param count Number of games to generate
   * @returns Array of MLB game data
   */
  generateGames(count: number): GameCardProps[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Generate game ID
      const gameId = this.generateGameId("MLB", i);
      
      // Generate spread (run line in MLB is typically -1.5)
      const spreadValue = 1.5;
      const spreadTeam = Math.random() > 0.5 ? homeTeam.name : awayTeam.name;
      
      // Generate over/under
      const overUnder = this.generateRandomOverUnder("MLB");
      ...
      const date = this.generateRandomDate(
        mockDataConfig.dateRanges.MLB.start,
        mockDataConfig.dateRanges.MLB.end
      );
      const time = this.generateRandomTime();
      const timezone = this.generateRandomTimezone();
      
      // Generate random network
      const network = this.generateRandomNetwork("MLB");
