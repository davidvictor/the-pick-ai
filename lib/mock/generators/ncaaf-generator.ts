...
import { NCAAF_TEAMS } from "@/lib/mock/data/teams/ncaaf";

/**
 * NCAAF game generator
 * This class generates mock NCAAF games
 */
export class NcaafGameGenerator extends GameGenerator {
  /**
  ...
  generateGames(count: number): GameCardProps[] {
    ...
    for (let i = 0; i < gamesToGenerate; i++) {
      ...
      // Generate game ID
      const gameId = this.generateGameId("NCAAF", i);
      
      // Generate spread (college football spreads can be larger)
      const spreadValue = (Math.floor(Math.random() * 30) + 1) / 2;
      const spreadTeam = Math.random() > 0.5 ? homeTeam.name : awayTeam.name;
      
      // Generate over/under
      const overUnder = this.generateRandomOverUnder("NCAAF");
      ...
      const date = this.generateRandomDate(
        mockDataConfig.dateRanges.NCAAF.start,
        mockDataConfig.dateRanges.NCAAF.end
      );
      const time = this.generateRandomTime();
      const timezone = this.generateRandomTimezone();
      
      // Generate random network
      const network = this.generateRandomNetwork("NCAAF");
