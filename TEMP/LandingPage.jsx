...
const ThemeToggle = () => {
  ...
  return (
    <Button
      ...
      className="fixed bottom-4 right-4 z-50 rounded-full bg-background/80 backdrop-blur-sm shadow-md"
...
/**
 * Landing page component that displays the betting lines visualization
 * and the login/signup box
 */
const LandingPage = () => {
  useEffect(() => {
    ...
    const getRandomWithStep = (min, max, step) => {
    ...
    const generateOdds = (isMoneyline = false) => {
    ...
    const createRows = () => {
      ...
      const itemsPerRow = 30; // Items per row
      ...
      for (let i = 0; i < rowCount; i++) {
        ...
        // Assign a random duration between 350s and 1800s
        const duration = getRandomDuration(350, 1800);
    ...
    const intervalId = setInterval(refreshSomeContent, 60000); // Refresh every 60 seconds
